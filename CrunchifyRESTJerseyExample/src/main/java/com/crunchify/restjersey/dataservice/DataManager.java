...
public class DataManager {
	...
	public static Response addPerson(Person person, Response response) {
		...
		PersonRepository.create(person); // ideally i would do this at the end i think TODO
	...
	public static Response deletePerson(int id, Response response) {
		PersonRepository.delete(id); // fix remove => something with entity recognition is wrong
		response.setStatus(true);
	...
	public static Person getPerson(int id) {
		return PersonRepository.findById(id); // if it is null, return val is null => will return http 404
	}
